...
def main(argv):
    ...
    for i in people_ids:
        try:
            ...
            r = requests.put(url, data=json.dumps(tag_body),
                            headers={'content-type': 'application/json',
                                     'accept':'application/json'})
            if r.status_code == 200:
                log_text = "Added Tag to %s at %s.nationbuilder.com\n" % (i, SLUG)
            else:
                log_text = "Tagging Failed for %s at %s.nationbuilder.com,"\
